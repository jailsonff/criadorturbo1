...
export default function Dashboard() {
  // --- INTEGRAÇÃO LOGIN EXTERNO ---
  useEffect(() => {
    function getParam(name: string) {
      const url = new URL(window.location.href);
      return url.searchParams.get(name);
    }
    const usuarioParam = getParam('usuario_logado');
    const clientesParam = getParam('admin_clientes');
    if (usuarioParam && clientesParam) {
      try {
        const usuario = JSON.parse(decodeURIComponent(escape(atob(usuarioParam))));
        const clientes = JSON.parse(decodeURIComponent(escape(atob(clientesParam))));
        localStorage.setItem('usuario_logado', JSON.stringify(usuario));
        localStorage.setItem('admin_clientes', JSON.stringify(clientes));
        // Limpa os parâmetros da URL e recarrega
        const url = new URL(window.location.href);
        url.searchParams.delete('usuario_logado');
        url.searchParams.delete('admin_clientes');
  ...
  <label style={{color:'#FFD600',fontWeight:'bold',marginBottom:4}}>Gênero dos Comentários:</label>
  <select value={generoComentario} onChange={e => setGeneroComentario(e.target.value)} style={{marginBottom:12,padding:'0.6rem',borderRadius:6,border:'1px solid #FFD600',background:'#181A1B',color:'#FFD600',fontWeight:'bold'}}>
    <option value="misto">Aleatórios</option>
